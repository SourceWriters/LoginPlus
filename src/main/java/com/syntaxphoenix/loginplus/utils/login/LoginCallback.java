        ...
        if (account.isPresent()) {
            if (config.isSessionsEnabled()) { // Handle Session-Logic
                ...
                if (pluginUtils.getSessionManager().validateSession(ip, player.getName())) {
                    ...
                    PlayerHelper.sendTitle(player, 20, config.getTitleTime() * 20, 20, MessagesConfig.title_login_session_title,
                        MessagesConfig.title_login_session_subtitle);
                    return;
                }
                pluginUtils.getSessionManager().resetSession(ip);
            }
            ...
            if (config.isCaptchaEnabled() && config.isCaptchaOnLogin() && this.checkCaptcha) {
                ...
                if (account.get().isPremium()) {
                    ...
                    PlayerHelper.sendTitle(player, 20, 100, 20, MessagesConfig.title_login_title, MessagesConfig.title_login_subtitle);
            ...
            if (config.isCaptchaEnabled() && config.isCaptchaOnRegister() && this.checkCaptcha) {
                ...
                PlayerHelper.sendTitle(player, 20, 100, 20, MessagesConfig.title_register_title, MessagesConfig.title_register_subtitle);
